...
// Обычная заглушка (без детей)
const Stub: React.FC<{ title: string }> = ({ title }) => {
  const { pathname } = useLocation();
  return (
    <div style={{ padding: 24 }}>
      <h1 style={{ margin: 0 }}>{title}</h1>
      <p style={{ opacity: 0.7, marginTop: 8 }}>Путь: <code>{pathname}</code></p>
    </div>
  );
};

// Заглушка-Layout c <Outlet/> для вложенных роутов
const LayoutStub: React.FC<{ title: string }> = ({ title }) => {
  ...
  return (
    <div style={{ padding: 24 }}>
      ...
      {/* Небольшое меню для проверки переходов */}
...
export const router = createBrowserRouter([
  {
    ...
    element: <BaseLayout />,
    children: [
      { index: true, element: <Stub title="Home (главная)" /> },
      { path: ROUTES.CATALOG,   element: <Stub title="Catalog" /> },
...

      {
        ...
        element: <ProtectedRoute><LayoutStub title="Profile" /></ProtectedRoute>,
        children: [
