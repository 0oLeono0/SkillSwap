...
describe('Filter utils', () => {
  ...
  describe('city helpers', () => {
    it('returns names for ids that exist in city options', () => {
      expect(mapCityIdsToCityNames(cityOptions, [3, 1])).toEqual([
        'Москва',
        'Воронеж'
      ]);
    });
    ...
    it('returns ids for names that exist in city options', () => {
      expect(
        mapCityNamesToCityIds(cityOptions, ['Санкт-Петербург', 'Москва'])
      ).toEqual([1, 2]);
    });
  });
  ...
  describe('skill helpers', () => {
    it('selects only skills belonging to a given group', () => {
      expect(selectedSkillsByGroup(skillGroups[0], [101, 999, 202])).toEqual([
        101
      ]);
    });
